...
class EspeakBackend(BaseEspeakBackend):
    ...
    def _init_tie(tie) -> Optional[str]:
        ...
        tie = str(tie)
        if len(tie) != 1:
            raise RuntimeError(f"explicit tie must be a single charcacter but is {tie}")
        return tie
    ...
    def supported_languages(cls):
        return {
            voice.language: voice.name for voice in EspeakWrapper().available_voices()
        }
    ...
    def _phonemize_aux(self, text, offset, separator, strip):
        ...
        for num, line in enumerate(text, start=1):
            ...
            line, has_switch = self._postprocess_line(line, num, separator, strip)
            output.append(line)
            if has_switch:
    ...
    def _postprocess_line(
        self, line: str, num: int, separator: Separator, strip: bool
    ) -> Tuple[str, bool]:
    ...
    def _phonemize_postprocess(
        self, phonemized, punctuation_marks, separator: Separator, strip: bool
    ):
        text = phonemized[0]
        ...
        phonemized = super()._phonemize_postprocess(
            text, punctuation_marks, separator, strip
        )
    ...
    def _flatten(phonemized) -> List:
        ...
        for i in range(len(phonemized[0])):
            flattened.append(
                list(itertools.chain(c for chunk in phonemized for c in chunk[i]))
            )
        return flattened
